...
public:
	...
	pointer allocate(size_type n , const void * hint= 0){ return _allocate(static_cast<difference_type>(n), static_cast<pointer>(0))};
	void deallocate(pointer p, size_tpye n){deallocate(p)};
	void construct(pointer p , const t & value){ _construct(p,value};
	allocator(size_tpye n , const void *  hint  = 0 ){return allocate(n,hint)};
	~allocator(void);	
};
